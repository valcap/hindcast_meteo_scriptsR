...
setEPS()
file_taylor_eps <- paste0(my_figs_dir,"/pippo.eps")
postscript(file_taylor_eps)
obs<-eobs_values_ts

# ERA5
pre <- era5_values_ts
taylor.diagram(obs, pre, add=F, col="red", ref.sd=TRUE, normalize=TD_normal, main="", grad.corr.lines=c(0.7,0.8), ngamma=5, pch=19, pcex=3., cex=1.5)

# MOLOCH
pre <- molo_values_ts
taylor.diagram(obs, pre, add=T, col="orange", ref.sd=TRUE, normalize=TD_normal, pch=19, pcex=3.0)

# BOLAM
pre <- bola_values_ts
taylor.diagram(obs, pre, add=T, col="blue", ref.sd=TRUE, normalize=TD_normal, pch=19, pcex=3.0)
