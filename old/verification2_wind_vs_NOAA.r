...
my_era5l_dir    <- paste0("/OCEANASTORE/progetti/spitbran/ERA5-Land/winds")
...
file_scores_csv <- paste0(my_output_dir,"/verification_scores_wind_era5l.csv")
...
###
  ...
  my_end_date       <- as.POSIXct(paste0(my_year,'-12-30 23:00:00'), format="%Y-%m-%d %H:%M:%S", tz="UTC")
###
  # 2. Check if files exists and load model data
  file_era5l_10u   <- paste0(my_era5l_dir,"/ERA5-Land_",my_year,"-hour_10u.nc")
  file_era5l_10v   <- paste0(my_era5l_dir,"/ERA5-Land_",my_year,"-hour_10v.nc")
  if ((file.exists(file_era5l_10u)==FALSE) || (file.exists(file_era5l_10v)==FALSE)) {
    print("ops missing ERA5-Land 10u or 10v file (or both)")
    quit()
  }
  # Start the clock!
  stm               <- proc.time()
  print(paste0("___Loading data from ERA5-Land 10u file___"))
  raster_era5l_10u <- brick(file_era5l_10u)
  print(paste0("___Loading data from ERA5-Land 10v file___"))
  raster_era5l_10v <- brick(file_era5l_10v)
  # Stop the clock!
  etm               <- proc.time() - stm
...
###
  ...
  for (my_obse_code in c(as.vector(my_anag_data$STATION_ID))) {
    ...
    if (my_year == '1999') {
      if ((my_obse_code == '07593099999')   || (my_obse_code == '16294099999')  || (my_obse_code == '16541099999') || (my_obse_code == '16337099999') || (my_obse_code == '16542099999') || (my_obse_code == '16224099999') || (my_obse_code == '16172099999') || (my_obse_code == '12920099999')) {
        next
      }
    }
    if (my_year == '2000') {
...
#####
    ...
    print(paste0("++++++Extracting data from ERA5-Land files"))
    stm             <- proc.time()
    era5l_10u       <- as.data.frame(raster::extract(raster_era5l_10u, SpatialPoints(cbind(obse_lon,obse_lat))))
    era5l_10v       <- as.data.frame(raster::extract(raster_era5l_10v, SpatialPoints(cbind(obse_lon,obse_lat))))
    etm             <- proc.time() - stm
    print(paste0("++++++OK in ",as.character(etm[3])," seconds"))
#####
    # 5. Model data: from zonal and meridional wind to wind speed and direction using the rWind package
    if ((any(is.na(era5l_10u))==TRUE) || (any(is.na(era5l_10v))==TRUE)) {
      print("++++++ops model data contain NA; skipping")
      next
    }
    data_from_rWind  <- uv2ds(t(era5l_10u),t(era5l_10v))
    era5l_wd         <- data_from_rWind[,1]
    era5l_wd         <- era5l_wd[25:nts]
    era5l_ws         <- data_from_rWind[,2]
    era5l_ws         <- era5l_ws[25:nts]
    my_era5l_data    <- as.data.frame(cbind(era5l_ws,era5l_wd))
#####
    ...
    cor_era5l        <- sprintf("%.3f",cor(era5l_ws,obse_ws, use = "complete.obs"))
    rmse_era5l       <- sprintf("%.3f",sqrt( mean( (era5l_ws-obse_ws)^2 , na.rm = TRUE ) ))
    mbias_era5l      <- sprintf("%.3f",(mean(era5l_ws, na.rm = TRUE))/(mean(obse_ws, na.rm = TRUE)))
    verify_era5l     <- verify(obse_ws,era5l_ws, frcst.type = "cont", obs.type = "cont",)
    me_era5l         <- sprintf("%.3f",verify_era5l$ME)
    mae_era5l        <- sprintf("%.3f",verify_era5l$MAE)
    print(paste0("++++++OK"))
#####
    # 7. Write the standard verification scores to the csv output file
    print(paste0("++++++Saving data to ",file_scores_csv,""))
    df <- data.frame(my_year,obse_code,obse_stat,obse_lat,obse_lon,obse_ele,my_ini_date,my_end_date,cor_era5l,rmse_era5l,mbias_era5l,me_era5l,mae_era5l)
