...
my_anag_file     <- paste0(my_working_dir,"/DATA/points025_italy_extent.csv")
...
# setEPS()
# postscript(paste0(my_figs_dir,"/Italy_virtual_stations.eps"))
# plot(extent(my_italy), main=paste0("Number of rain-gauges ", length(pointCoordinates)), add=F, col='white', lwd=1, xlab="Longitude [degree]", ylab="Latitude [degree]")
# plot(pointCoordinates, pch=19, cex=0.5, add=T)
...
###
  ...
  file_grip   <- paste0("DATA/GRIPHO/tp/gripho-v2-3km_year_tp_",my_year,".nc")
#  file_grip   <- paste0("DATA/GRIPHO/tp/gripho-v2-10km_year_tp_",my_year,".nc")
...
###
  ...
  names(obse_resam)     <- as.Date(names(obse_resam),format="X%Y.%m.%d")
...
###
  ...
  obse_values <- as.data.frame(extract(obse_resam, pointCoordinates))[,1]
  ...
  #
  obse_values <- replace(obse_values,obse_values<cutoff_NA, NA)
